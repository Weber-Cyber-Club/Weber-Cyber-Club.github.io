...
date = "2026-01-11T19:33:51-07:00"
author = "The Professor"
cover = "/assets/ProfessorsBanner.png"
categories = ["Binary Exploitation"]
description = "The goal of this lab is to give students an introduction into binary exploitation. While this is not the most basic form of exploitation, it is a form that can give some good insight into how a computer works, how a processor runs through code, and how different code security features work."
...
![ROP output](/assets/rop/ropoutput.png)

Nothing crazy is happening so far, just a normal output. Let's take a look at this code in a disassembler to see if there are any flaws that we could take advantage of. A good disassembler that should be in your install is called Ghidra. You can go up to your search bar or applications section, look for Ghidra and run it, or you can run it from your terminal with:

```
ghidra
```

Now to create a project we will go to File > New Project, then click “Non-Shared Project” and click next. Keep the project directory the same, and name the project ROP-Challenge, then click finish.

Then, we will go to File > Import File, then navigate to your `~/Club-Challenges/Labs/ROP` file or where you stored it, import the file, and click next on the pop up that appears. Now, once we are back in the projects window, just double click on ROP.

Click `yes` on analyze and keep the default values, then click analyze. You should end up at a screen that looks like this:

![Ghidra](/assets/rop/ghidra.png)

Now, on the left middle of the screen there is a section called `Symbol Tree`. We want to go to `Symbol Tree` and click the arrow next to functions, then scroll down and click on main.

![main](/assets/rop/main.png)
...
![segfault](/assets/rop/segfault.png)
...
![leak](/assets/rop/leak.png)

We now have the memory leak that we need to get through the second part of our exploitation of this system. We now need to use this leak to find which version of `Libc` we will need to attack. Let's go to [this site](https://libc.blukat.me/), input our memory address, and see what it tells us.

![blukat](/assets/rop/blukat.png)

We can now see that the version of `Libc` could be one of a few variations of `libc6_2.39`, in my experience, the different versions of Ubuntu variants don't matter, but I just get the most recent one. Click on the one for `Ubuntu 8.6 amd64`, and right click the download link to copy the link. Here is the link if you can't copy it, `http://libc.blukat.me/d/libc6_3.39-0ubuntu8.6_amd64.so`. Now, we will run `wget` from within the docker container to pull down this `Libc` shared object file.
...
![success](/assets/rop/success.png)
